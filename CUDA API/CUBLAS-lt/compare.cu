...
void intializeMatrix(std::vector<float>& matrix, int rows, int cols) {
...
// Function to perform warmp and benchmark runs
float benchmark_kernel(std::function<void()> kernel_func, int warmup_runs, int benchmark_runs) {
    ...
    float avg_time = std::accumulate(times.begin(), times.end(), 0.f) / benchmark_runs;
    return avg_time;
}


int main() {
    std::cout << "matrix size: " << M << "x" << K << " * " << N << std::endl;
    ...
    // Initialize the matrices and copy them to the devices
    intializeMatrix( h_A, M, K);
    intializeMatrix( h_B, K, N);
    ...
    // Initialize the handles and the parameters
    ...
    CHECK_CUBLAS(cublasLtMatrixLayoutCreate(&Bdesc, CUDA_R_32F, N, K, N)); // Original M K M
    CHECK_CUBLAS(cublasLtMatrixLayoutCreate(&Cdesc, CUDA_R_32F, N, M, N)); // Original M K M
    float cublaLt_fp32_time = benchmark_kernel([&]() {
        CHECK_CUBLAS(
            cublasLtMatmul(
                cublasLt_handle, 
                operationDesc, 
    ...
    std::cout << "cubLASLt FP32 Average time: " << cublaLt_fp32_time << "ms" << std::endl;
