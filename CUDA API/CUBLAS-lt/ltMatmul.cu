...
int main() {
    ...
    // Setup matrix descriptors for FP32
    ...
    // Setup Matrix descriptors for FP16
...
CHECK_CUBLAS(
    cublasLtMatmul(
        handle,                // Handle to cuBLASLt context
        matmulDesc_fp32,       // Matrix multiplication descriptor
        &alpha,                // Scaling factor for A*B
        d_B_fp32,              // Pointer to matrix B (device memory)
        matB_fp32,             // Descriptor for matrix B
        d_A_fp32,              // Pointer to matrix A (device memory)
        matA_fp32,             // Descriptor for matrix A
        &beta,                 // Scaling factor for existing C
        d_C_fp32,              // Pointer to matrix C (device memory, input)
        matC_fp32,             // Descriptor for matrix C
        d_C_fp32,              // Pointer to matrix C (device memory, output)
        matC_fp32,             // Descriptor for matrix C
        nullptr,               // Pointer to a workspace buffer (optional)
        nullptr,               // Algorithm configuration (optional)
        0,                     // Size of workspace buffer (optional)
        0                      // Algorithm preference flags (optional)
    ));
