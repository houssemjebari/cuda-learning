...
int main() {
    ...
    // Print out the problem dimensions
    ...
    // Set descriptor details
    CHECK_CUDNN(cudnnSetFilter4dDescriptor(kernelDesc, CUDNN_DATA_FLOAT,
                                           CUDNN_TENSOR_NCHW, 
                                           outChannels, inChannels, kernelSize, kernelSize));

    CHECK_CUDNN(cudnnSetTensor4dDescriptor(inputDesc, CUDNN_TENSOR_NCHW, 
                                           CUDNN_DATA_FLOAT, 
                                           batchSize, inChannels, height, width));

    CHECK_CUDNN(cudnnSetTensor4dDescriptor(outputDesc, CUDNN_TENSOR_NCHW, 
                                           CUDNN_DATA_FLOAT, 
    ...
    for (int i = 0 ; i < returnedAlgoCount ; i++) {
        std::cout << "Algorithm: " << perfResults[i].algo
                  << ", Time: "    << perfResults[i].time
                  << ", ms, Memory: "    << perfResults[i].memory
    ...
    for (int i = 0; i < returnedAlgoCount; ++i) {
        if (perfResults[i].status == CUDNN_STATUS_SUCCESS) {
            // This is sorted by best time first, so pick the first
    ...
    if (!foundAlgo) {
        // fallback to a default if none succeeded
    ...
    float totalTime_cudnn = 0.f;
    float totalTime_naive = 0.f;
