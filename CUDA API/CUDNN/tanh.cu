...
int main() {
    ...
    // Naive CUDA Kernel Setup 
        ...
        for (int i = 0; i < num_benchmark; ++i) {
            CHECK_CUDA(cudaEventRecord(start)); // Record the start time
            naiveTanhKernel<<<grid, block>>>(d_input, d_output_naive, tensor_size);
            CHECK_CUDA(cudaEventRecord(stop)); // Record the stop time
            CHECK_CUDA(cudaEventSynchronize(stop)); // Wait for the event to complete
            CHECK_CUDA(cudaEventElapsedTime(&naive_times[i], start, stop)); // Measure elapsed time
        }
    ...
    float avg_naive_time = 0.f, avg_cudnn_time = 0.f;
    for (int i = 0; i < num_benchmark; ++i) {
